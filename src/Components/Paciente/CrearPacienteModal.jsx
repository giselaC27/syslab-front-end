...
const CrearPacienteModal = ({ isOpen, onClose, onSave, institutions, empresas, tiposPacientes, generos, paciente, actualizaInfoTurno }) => {
  ...
  useEffect(() => {
    if (paciente) {
      setFormValues({
        cedulaIdentidad: paciente.cedulaIdentidad || '',
      });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isFormValid) {
      ...
      try {
        ...
        setError('Error creando paciente: ' + (error.response?.data || error.message));
        setIsLoading(false);
      }
    }
  };
