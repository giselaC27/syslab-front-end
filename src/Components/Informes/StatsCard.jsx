...
const StatsCards = ({ turnos }) => {
  ...
  const turnosPagados= turnos.filter(turno => turno.estado === 'PAGADO').length;
  ...
  const porcentajePagados = (turnosPagados / totalTurnos) * 100 || 0;

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
      <div className="p-4 bg-white shadow rounded-lg">
        <h3 className="text-lg font-semibold">Turnos Totales</h3>
      ...
      <div className="p-4 bg-white shadow rounded-lg">
        <h3 className="text-lg font-semibold">Turnos Pagados</h3>
        <p className="text-2xl">{turnosPagados}</p>
        <p className="text-sm">{porcentajePagados.toFixed(2)}%</p>
      </div>
      <div className="p-4 bg-white shadow rounded-lg">
